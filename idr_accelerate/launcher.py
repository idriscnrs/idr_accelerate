...
from argparse import ArgumentParser, REMAINDER, _HelpAction
import idr_torch
import json
...
def make_dist_config(user_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    processes_per_node = max(len(idr_torch.gpu_ids), 1)  # default to one process per node in case of cpu job
